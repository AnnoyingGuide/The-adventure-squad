    ...
    public class grabbag1 : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            if (UseItem(player))
            {
                ...
                Companion tg = 
                terraguardians.WorldMod.SpawnCompanionNPC(player.Bottom, NewContainer.Nathan, Mod.Name);
                Main.NewText("After opening the bag someone fell out");
                Item.SetDefaults(0);
                return true;
            }
            Main.NewText("You've already let Nathan out of this bag");
            return false;
        }
